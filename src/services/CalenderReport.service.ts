...
class CalenderReportService {
  ...
  public async getCalenderReport(
    ...
    if (!roomData) {
      throw new ApiError(
        404,
        `Room with id ${roomId} not found in property ${propertyId} for this tenant.`
      );
    }
    ...
    for (const day of dateRange) {
      ...
      let dayStatus: "AVAILABLE" | "BOOKED" | "PENDING" | "UNAVAILABLE" =
        "AVAILABLE";
      let dayPrice = roomData.basePrice;

      const activeBooking = bookings.find(
        (b) => day >= b.checkIn && day < b.checkOut
      );
      if (activeBooking) {
        dayStatus =
          activeBooking.status === BookingStatus.MENUNGGU_PEMBAYARAN
            ? "PENDING"
            : "BOOKED";
      } else {
        const availabilityRecord = rawAvailabilityData.find(
  ...
  public async getAggregatedPropertyReport(
    tenantId: string,
    propertyId: string,
    startDate: Date,
    endDate: Date
    ...
    for (const day of dateRange) {
      ...
      const availableRecordsCount = recordsForDate.filter(
        (r) => r.isAvailable
      ).length;
      ...
      const totalAvailableCount =
        availableRecordsCount + unrecordedAvailableCount;
...
export default new CalenderReportService();
