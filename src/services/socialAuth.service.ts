...
class SocialAuthService {
  private async findOrCreateUser(
    ...
    if (!email) {
      throw new ApiError(
        400,
        `Email tidak dapat diambil dari profil ${provider}. Pastikan email Anda publik.`
      );
    }
    ...
    if (userByEmail) {
      throw new ApiError(
        409,
        "Email sudah terdaftar dengan metode lain (misalnya, password). Silakan login dengan cara tersebut."
      );
    }
