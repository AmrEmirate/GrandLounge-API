...
class PublicPropertyService {
    ...
    private buildAvailabilityMap(availabilities: RoomAvailability[]): Map<string, { lowestPrice: number; isAvailable: boolean }> {
        const availabilityMap = new Map<string, { lowestPrice: number; isAvailable: boolean }>();
    ...
    private generateDateRangeResult(
        startDate: Date,
        endDate: Date,
        map: Map<string, { lowestPrice: number; isAvailable: boolean }>,
        defaultPrice: number,
        ...
        for (let day = new Date(startDate); day <= endDate; day.setDate(day.getDate() + 1)) {
            ...
            const priceWithSeason = this.applyPeakSeason(defaultPrice, day, peakSeasons);
    ...
    public async getMonthlyAvailability(propertyId: string, month: number, year: number) {
        ...
        const availabilityMap = this.buildAvailabilityMap(roomAvailabilities);
        const defaultLowestPrice = Math.min(...rooms.map(r => r.basePrice));

        return this.generateDateRangeResult(startDate, endDate, availabilityMap, defaultLowestPrice, peakSeasons);
    }
