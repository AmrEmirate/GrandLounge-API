...
import { PrismaClient, TokenPurpose, User } from "@prisma/client";
...
class TokenServiceClass {
  ...
  private sendEmailChangeConfirmationEmail(
    user: User,
    token: string,
    newEmail: string
  ) {
  ...
  public async validateAndUseToken(token: string, purpose: TokenPurpose) {
