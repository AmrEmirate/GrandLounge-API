...
class PeakSeasonService {
  ...
  public async updateSeason(
    id: string,
    data: Prisma.PeakSeasonUncheckedUpdateInput
  ): Promise<PeakSeason> {
    ...
    if (data.startDate) {
      adjustedUpdateData.startDate = startOfDay(
    ...
    if (
      adjustedUpdateData.startDate &&
      adjustedUpdateData.endDate &&
      isBefore(
        new Date(adjustedUpdateData.endDate as string),
        new Date(adjustedUpdateData.startDate as string)
      )
    ) {
