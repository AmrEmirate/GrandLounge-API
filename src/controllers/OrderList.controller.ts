...
class OrderListController {
  public async orderList(
    ...
    try {
      ...
      const orderlist = await OrderListService.getOrderList(userId, filter);
  ...
  public async tenantTransactionList(
    ...
    try {
      ...
      const transactions = await OrderListService.getTenantTransactions(
        tenantId,
        filter
      );
  ...
  public async completeOrder(
    ...
    try {
      ...
      const updatedOrder = await OrderListService.completeOrder(
        userId,
        bookingId
      );
  ...
  public async pendingConfirmationList(
    ...
    try {
      const tenantId = (req.user as any).tenant.id;
      const transactions = await OrderListService.getPendingConfirmation(
        tenantId
      );
      res.status(200).json({
        success: true,
